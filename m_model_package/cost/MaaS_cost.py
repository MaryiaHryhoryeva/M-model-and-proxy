...
############# costs #############
base_fare = 2.5     # base passenger fare
fare_per_km = 1.72  # passenger fare per km

rh_service_fee = 0.2    # RH service fee from each ride

driver_price_km = 1.37   # expenses of driver per km
...
if __name__ == "__main__":
    ...
    total_km = sum(np.multiply(o_RHI, l_RHI)) + sum(np.multiply(o_RH, l_RH))
    pass_pay = total_km * fare_per_km + sum(o_RHI) * base_fare
    print("total passengers payments during the simulation: " + str(pass_pay))
    print("company profit: " + str(pass_pay * rh_service_fee))
    print("total drivers profit: " + str(pass_pay * (1 - rh_service_fee) - total_km * driver_price_km))
    print("profit per driver: " + str((pass_pay * (1 - rh_service_fee) - total_km * driver_price_km) / (n_I[0] + n_RHI[0] + n_RH[0])))
