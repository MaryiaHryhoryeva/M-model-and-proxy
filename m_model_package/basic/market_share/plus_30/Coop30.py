...
if __name__ == "__main__":
    ...
    n_PAS['uber'].append(0)
    n_I['uber'].append(156)
    n_RHI['uber'].append(65)
    n_RH['uber'].append(130)

    n_PAS['lyft'].append(0)
    n_I['lyft'].append(234)
    n_RHI['lyft'].append(65)
    n_RH['lyft'].append(130)
    ...
    result11 = [i1 * i2 for i1, i2 in zip(o_RHI['uber'], l_RHI['uber'])]
    result21 = [i1 * i2 for i1, i2 in zip(o_RH['uber'], l_RH['uber'])]
    print(sum(result11))
    print(sum(result21))
    print("TTD uber: " + str(sum(result11) + sum(result21)))

    result12 = [i1 * i2 for i1, i2 in zip(o_RHI['lyft'], l_RHI['lyft'])]
    result22 = [i1 * i2 for i1, i2 in zip(o_RH['lyft'], l_RH['lyft'])]
    print(sum(result12))
    print(sum(result22))
    print("TTD lyft: " + str(sum(result12) + sum(result22)))

    result3 = [i1 * i2 for i1, i2 in zip(o_PV, l_PV)]
    print("TTD PV: " + str(sum(result3)))

    print("TTD total: " + str(sum(result11) + sum(result21) + sum(result12) + sum(result22) + sum(result3)))

    print("utilisation rate of vehicles:  " + str(
        ((max_t * 780 / delta_t) - (sum(n_I['uber']) + sum(n_I['lyft']))) / (max_t * 780 / delta_t)))
