############################################################################################################################################################
############## Cancellation of matched requests (NOT requests in the queue) where idle distance is bigger than a threshold ################################
############################################################################################################################################################
...
def sigma_idle_calcul():        # calculation of st dev of idle veh-s using a regression model from microscopic simulation
    if n_PAS[-1] > 0:   # if there are queued passengers
        ...
        demand_count_norm = (lmd_RH[int(i / delta_t)] / (max_x * max_y)) * 40000 * 40  # normalizing the demand (40000 = 200 * 200, which is a length of block in microsim) and multiplying by 40 as in microsim regression is done for demand normalized over 40 sec while here the time is calculated in 1 sec
        ...
        Xdf = pd.DataFrame(X)       # transform to dataframe
        x_ = PolynomialFeatures(degree=2, include_bias=False).fit_transform(Xdf)        # transform data to use polynomial regression
        model = pickle.load(open('/Users/maryia/PycharmProjects/SimulationTrial/estimation models/st_dev_idle_dist_model', 'rb'))     # use regression model created in micro-simulation
        sigma_idle_ = model.predict(x_)     # prediction
        sigma_idle.append(sigma_idle_[0] * 200)          # multiplying by 200 as sigma is calculated for a block of 200 m
        print("sigma idle: " + str(sigma_idle[-1]))
        if sigma_idle[-1] < 0:
...
def idle_dist_calcul():     # calculation of avg trip length of idle veh-s using a regression model from microscopic simulation
    if n_PAS[-1] > 0:
        ...
        Xdf = pd.DataFrame(X)       # transform to dataframe
        x_ = PolynomialFeatures(degree=2, include_bias=False).fit_transform(Xdf)        # transform data to use polynomial regression
        model = pickle.load(open('/Users/maryia/PycharmProjects/SimulationTrial/estimation models/idle_dist_model', 'rb'))     # use regression model created in micro-simulation
        idle_dist_ = model.predict(x_)    # prediction
        l_RHI.append(idle_dist_[0] * 200)
        print("l_RHI: " + str(l_RHI[-1]))
...
def init_lmb(max_time):      # demand initialization
    ...
    for i1 in range(0, max_time // delta_t + 1):
        ...
        elif (dem_vary_t // delta_t + 1) / 4 < i1 <= ((dem_vary_t // delta_t + 1) / 4 + (dem_vary_t // delta_t + 1) / (4 * 6)):
            lmd_RH.append(0.35)
            lmd_PV.append(19.5)
...
def n_PAS_and_n_I():                                            # Calculation of accumulation of idle veh-s and waiting passengers to be matched
    if (delta_t * o_RH[-1] + n_I[-1]) < \
        ...
        cancel.append(dummy1 - o_PAS[-1])                   # add in cancellation list the difference between the initially calculated pass. outflow and the new one
...
if __name__ == "__main__":
    ...
    with open('/Users/maryia/PycharmProjects/SimulationTrial/m_model_package/basic/one_comp/length_1_comp.csv', 'w') as f:
    ...
    with open('/Users/maryia/PycharmProjects/SimulationTrial/m_model_package/basic/one_comp/speed_1_comp.csv', 'w') as f:
        # create the csv writer
        writer = csv.writer(f)
