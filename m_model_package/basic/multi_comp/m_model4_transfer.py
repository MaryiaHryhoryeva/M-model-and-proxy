...
def visualization():  ################## Visualization #################
    ...
    def dem():
        ...
        u = queue_common_total['didi']
        p = queue_common_total['bolt']
        plt.plot(x, 'b')
        plt.plot(y, 'cornflowerblue')
        plt.plot(u, 'aqua')
        plt.plot(p, 'silver')
        ...
        u = dem_served_by_competitor['didi']
        p = dem_served_by_competitor['bolt']
        plt.plot(x, 'b')
        plt.plot(y, 'cornflowerblue')
        plt.plot(u, 'aqua')
        plt.plot(p, 'silver')
...
if __name__ == "__main__":
    ...
    v.append(calculate_V_MFD(
        n_RHI['uber'][-1] + n_RH['uber'][-1] + n_RHI['bolt'][-1] + n_RH['bolt'][-1] + n_RHI['didi'][-1] + n_RH['didi'][-1] + n_RHI['lyft'][-1] + n_RH['lyft'][-1] + n_PV[-1] + n_I_mov['uber'][-1] + n_I_mov['bolt'][-1] + n_I_mov['didi'][-1] +
    ...
    for i in range(delta_t, max_t, delta_t):
        ...
        v.append(
            calculate_V_MFD(n_RHI['uber'][-1] + n_RH['uber'][-1] + n_RHI['bolt'][-1] + n_RH['bolt'][-1] + n_RHI['didi'][-1] + n_RH['didi'][-1] + n_RHI['lyft'][-1] + n_RH['lyft'][-1] + n_PV[-1] +
                            n_I_mov['uber'][-1] + n_I_mov['bolt'][-1] + n_I_mov['didi'][-1] + n_I_mov['lyft'][-1]))
        print("v: " + str(v[-1]))
    ...
    print("total accum of veh: " + str(
        sum(num_mov_veh['uber']) + sum(num_mov_veh['lyft']) + sum(num_mov_veh['bolt']) + sum(num_mov_veh['didi']) + sum(
            n_PV)))
    print("total cancellation: " + str(
        (sum(cancel['uber']) + sum(cancel['lyft']) + sum(cancel['bolt']) + sum(cancel['didi'])) * delta_t))
    ...
    print("total avg pass wait time: " + str((sum(pass_wait_time['lyft']) + sum(pass_wait_time['uber']) + sum(
        pass_wait_time['bolt']) + sum(pass_wait_time['didi'])) / (
                                                     len(pass_wait_time['lyft']) + len(pass_wait_time['didi']) + len(
                                                 pass_wait_time['bolt']) + len(pass_wait_time['uber']))))
