...
if __name__ == "__main__":
    ...
    v.append(calculate_V_MFD(
        n_RHI['uber'][-1] + n_RH['uber'][-1] + n_RHI['didi'][-1] + n_RH['didi'][-1] + n_RHI['lyft'][-1] + n_RH['lyft'][-1] + n_PV[-1] + n_I_mov['uber'][-1] + n_I_mov['didi'][-1] +
    ...
    for i in range(delta_t, max_t, delta_t):
        ...
        v.append(
            calculate_V_MFD(n_RHI['uber'][-1] + n_RH['uber'][-1] + n_RHI['didi'][-1] + n_RH['didi'][-1] + n_RHI['lyft'][-1] + n_RH['lyft'][-1] + n_PV[-1] +
                            n_I_mov['uber'][-1] + n_I_mov['didi'][-1] + n_I_mov['lyft'][-1]))
        print("v: " + str(v[-1]))
    ...
    print("total avg pass wait time: " + str((sum(pass_wait_time['lyft']) + sum(pass_wait_time['didi']) + sum(pass_wait_time['uber'])) / (len(pass_wait_time['lyft']) + len(pass_wait_time['didi']) + len(pass_wait_time['uber']))))
