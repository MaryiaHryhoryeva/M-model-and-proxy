...
def visualization():  ################## Visualization #################
    ...
    def countcurve_bolt():
        plt.subplots(1, 1, constrained_layout=True)
        plt.subplot(1, 1, 1)
        x = np.cumsum(o_PAS['bolt'])
        y = np.cumsum(o_RHI['bolt'])
        z = np.cumsum(o_RH['bolt'])
        j = np.cumsum(lmd_RH['bolt'])
        p = np.cumsum(o_PAS_total['bolt'])
        plt.plot(x, 'magenta', label='o_PAS')
        plt.plot(y, 'k', label='o_RHI')
        plt.plot(z, 'g', label='o_RH')
        plt.plot(j, 'r', label='lmb_RH')
...
if __name__ == "__main__":
    ...
    n_PAS['lyft'].append(0)
    n_I['lyft'].append(510)
    n_RHI['lyft'].append(210)
    n_RH['lyft'].append(330)

    n_PAS['uber'].append(0)
    n_I['uber'].append(480)
    n_RHI['uber'].append(180)
    n_RH['uber'].append(330)

    n_PAS['didi'].append(0)
    n_I['didi'].append(420)
    n_RHI['didi'].append(120)
    n_RH['didi'].append(270)

    n_PAS['bolt'].append(0)
    n_I['bolt'].append(390)
    n_RHI['bolt'].append(90)
    n_RH['bolt'].append(270)
    ...
    v.append(calculate_V_MFD(
        n_RHI['didi'][-1] + n_RH['didi'][-1] + n_RHI['bolt'][-1] + n_RH['bolt'][-1] + n_RHI['uber'][-1] + n_RH['uber'][-1] + n_RHI['lyft'][-1] + n_RH['lyft'][-1] + n_PV[-1] + n_I_mov['uber'][-1] +
        n_I_mov['lyft'][-1] + n_I_mov['bolt'][-1] + n_I_mov['didi'][-1]))  # calculation of speed
    ...
    print("n: " + str(n_I['uber'][-1] + n_RHI['uber'][-1] + n_RH['uber'][-1] +
                      ...
                      n_I['bolt'][-1] + n_RHI['bolt'][-1] + n_RH['bolt'][-1] +
    ...
    for i in range(delta_t, max_t, delta_t):
        ...
        v.append(
            calculate_V_MFD(n_RHI['didi'][-1] + n_RH['didi'][-1] + n_RHI['bolt'][-1] + n_RH['bolt'][-1] + n_RHI['uber'][-1] + n_RH['uber'][-1] + n_RHI['lyft'][-1] + n_RH['lyft'][-1] + n_PV[-1] +
                            n_I_mov['uber'][-1] + n_I_mov['bolt'][-1] + n_I_mov['lyft'][-1] + n_I_mov['didi'][-1]))
        print("v: " + str(v[-1]))
    ...
    print("total accum of veh: " + str(sum(num_mov_veh['uber']) + sum(num_mov_veh['lyft']) + sum(num_mov_veh['bolt']) + sum(num_mov_veh['didi']) + sum(n_PV)))
    print("total cancellation: " + str((sum(cancel['uber']) + sum(cancel['lyft']) + sum(cancel['bolt']) + sum(cancel['didi'])) * delta_t))
    ...
    accum_o_PAS_total_bolt = np.cumsum(o_PAS_total['bolt'])
    accum_lmd_RH_bolt = np.cumsum(lmd_RH['bolt'])
    for t10 in range(0, len(accum_lmd_RH_bolt)):
        for t11 in range(0, len(accum_o_PAS_total_bolt)):
            if accum_o_PAS_total_bolt[t11] - 0.05 <= accum_lmd_RH_bolt[t10] <= accum_o_PAS_total_bolt[t11] + 0.05:
                pass_wait_time['bolt'].append((t11 - t10) * delta_t)

    print("avg pass wait time bolt: " + str(sum(pass_wait_time['bolt']) / len(pass_wait_time['bolt'])))

    print("total avg pass wait time: " + str((sum(pass_wait_time['lyft']) + sum(pass_wait_time['uber']) + sum(pass_wait_time['bolt']) + sum(pass_wait_time['didi'])) / (
                len(pass_wait_time['lyft']) + len(pass_wait_time['didi']) + len(pass_wait_time['bolt']) + len(pass_wait_time['uber']))))

    print("utilisation rate of vehicles:  " + str(
        ((max_t * 600 / delta_t) - (sum(n_I['bolt']) + sum(n_I['didi']) + sum(n_I['uber']) + sum(n_I['lyft']))) / (max_t * 600 / delta_t)))
    print("accum of idle stopped:  " + str(sum(n_I['bolt']) + sum(n_I['didi']) + sum(n_I['uber']) + sum(n_I['lyft'])))
