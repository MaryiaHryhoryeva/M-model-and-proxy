...
cancel = []         # number of cancelled requests
...
def o_MFD(l_RH, l_PV, i):  # calculation of the outflow. includes technique to prevent negative accumulation of vehicles
    if i == delta_t:
    ...
    o_RH.append(min((n_RH[-1] + alpha * (m_RH[-1] / l_RH_s[-1] - n_RH[-1])) * v[-1] / l_RH[int(i / delta_t)],
...
def sigma_idle_calcul(i):  # calculation of st dev of idle veh-s using a regression model from microscopic simulation
...
def steady_dist_l_s(l_RH, l_PV, i):  # calculation of avg remaining distance t obe traveled in steady state
    sigma_idle_calcul(i)
...
def idle_dist_calcul(i):  # calculation of avg trip length of idle veh-s using a regression model from microscopic simulation
    if n_PAS[-1] > 0:
...
def init_lmb(max_time):  # demand initialization
    dem_vary_t = 4000  # time duration when demand is variable
...
def n_PAS_and_n_I(i):  # Calculation of accumulation of idle veh-s and waiting passengers to be matched
    if (delta_t * o_RH[-1] + n_I[-1]) < \
        ...
        n_PAS.append(
            max(delta_t * (lmd_RH[int(i / delta_t)] - o_RH[-1]) - n_I[-1] + n_PAS[-1], 0))  # accumulation of passengers
        ...
        n_I.append((dummy1 - o_PAS[-1]) * delta_t)  # accumulation of idle vehicles
        cancel.append(dummy1 - o_PAS[-1])

    elif (delta_t * o_RH[-1] + n_I[-1]) >= \
            (delta_t * lmd_RH[int(i / delta_t)] + n_PAS[
                -1]):  # case when demand is lower or equal than available vehicles
        o_PAS.append(
            lmd_RH[int(i / delta_t)] + n_PAS[-1] / delta_t)  # outflow of passengers = inflow of idle moving vehicles
        dummy1 = o_PAS[-1]
        ...
        n_I.append(max(delta_t * (o_RH[-1] - lmd_RH[int(i / delta_t)]) - n_PAS[-1] + n_I[-1],
                       0))  # accumulation of idle vehicles
        n_I[-1] = n_I[-1] + (dummy1 - o_PAS[-1]) * delta_t
        n_PAS.append(0)  # accumulation of passengers
        cancel.append(dummy1 - o_PAS[-1])
...
def m_model_sim(n_RH_total):
    ...
    print("iter: 0")
    n_PAS.append(0)
    n_I.append(round(n_RH_total * 0.25))
    n_RHI.append(n_RH_total - round(n_RH_total * 0.25) - round(n_RH_total * 2 / 3))
    n_RH.append(round(n_RH_total * 2 / 3))
    n_PV.append(23559)
    ...
    idle_dist_calcul(i)  # prediction of avg idle distance using regression
    # l_RHI.append(500)       #
    ...
    np.random.seed(5740)
    l_RH = np.random.normal(5740, sigma_occup, max_t // delta_t + 1)  # random avg trip length from norm distribution
    ...
    v.append(calculate_V_MFD(n_RHI[-1] + n_RH[-1] + n_PV[-1]))  # calculation of speed
    ...
    print(l_RH)
    steady_dist_l_s(l_RH, l_PV, i)  # avg remaining distance to be traveled in steady state
    ...
    for i in range(delta_t, max_t, delta_t):
        ...
        v.append(calculate_V_MFD(n_RHI[-1] + n_RH[-1] + n_PV[-1]))
        print("v: " + str(v[-1]))
        prod_n_I_mov.append(n_I_mov[-1] * v[-1] * delta_t)
        ...
        num_mov_veh.append(n_RHI[-1] + n_RH[-1])
        print("# of RH vehicles in the system: " + str(num_veh[-1]))
    o_MFD(l_RH, l_PV, max_t)
    n_PAS_and_n_I(max_t)  # accumulation of idle stopped vehicles and passengers waiting to be matched. Must be placed after calculation of outflow and before calculation of vehicle accumulation
    n_I_mov.append(round(n_I[-1] * frac_n_I_mov))
    n_I_stop.append(n_I[-1] - n_I_mov[-1])

    return o_PAS, o_RHI, l_RHI, n_RHI, v
...
if __name__ == "__main__":
    ...
    with open('/Users/maryia/PycharmProjects/SimulationTrial/m_model_package/basic/length_1_comp.csv', 'w') as f:
    ...
    with open('/Users/maryia/PycharmProjects/SimulationTrial/m_model_package/basic/speed_1_comp.csv', 'w') as f:
        # create the csv writer
        writer = csv.writer(f)
    ...
    obj_fun1 = [0] * len(n_RH_total)
    obj_fun2 = [0] * len(n_RH_total)
    obj_fun3 = [0] * len(n_RH_total)

    for i in range(0, len(n_RH_total)):
        oPAS, oRHI, lRHI, nRHI, speed = m_model_sim(n_RH_total[i])
        for j in range(0, len(lRHI)):
            obj_fun1[i] += fare * oPAS[j] * delta_t - wage * oPAS[j] * delta_t - oRHI[j] * lRHI[j] * charge * delta_t
            #print(oRHI[j] * lRHI[j])
            obj_fun2[i] += fare * oPAS[j] * delta_t - wage * oPAS[j] * delta_t - nRHI[j] * speed[j] * charge * delta_t
            #print(nRHI[j] * speed[j])
            obj_fun3[i] += fare * oPAS[j] * delta_t - wage * oPAS[j] * delta_t - nRHI[j] * charge * delta_t
            #print()


    #for i in range(0, len(l_RHI)):
    #    obj_fun1 += fare * o_PAS[i] - wage * o_PAS[i] - o_RHI[i] * l_RHI[i] * charge
    ...
    plt.subplot(3, 1, 1)
    x = obj_fun1
    plt.plot(list(range(len(n_RH_total))), x, 'g')
    plt.xlabel('fleet size')
    plt.ylabel('Objective function')
    plt.xticks(list(range(len(n_RH_total))), n_RH_total)
    plt.title('fare = ' + str(fare) + ', charge = ' + str(charge) + ', wage = ' + str(wage))
