...
def initialization_dem(num_veh, demand_count, discarded_time):
    ...
    for key in num_veh:  # initialization of demands
        ...
        for i in range(1, random.randint(round(10 * num_veh[key]) + 1, round(13 * num_veh[key]) + 1)):  # 1 * fleet <= demand <= 8 * fleet
...
for iter in range(0, 200):
    ...
    companies = ['uber']
    num = random.randrange(200, 1000, 2)  # number of vehicles of each company
    #num = 400
...
#df.to_csv('/Users/maryia/PycharmProjects/SimulationTrial/micro_sim/micro_sim_data2.csv')
...
df3 = pd.DataFrame(dataframe_dict3)
df3.to_csv('/Users/maryia/PycharmProjects/SimulationTrial/micro_sim/microsim_result_data/glob_idle_dist_to_queued_dem_avg2.csv')